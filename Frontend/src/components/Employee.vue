<template>
  <v-layout align-start>
    <v-flex>
        ...
        <v-dialog v-model="adModal" max-width="290">
          <v-card>
            ...
            <v-card-text>
              ...
              <span v-if="adAction == 2"><strong> Dactivate </strong> </span>
              the employee <strong>{{ adName }}</strong>
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn
      ...
      <v-btn color="error" danger class="mb-2" @click="removeAll()"
        >Delete all inactive users</v-btn
      >
      <v-data-table
        :headers="headers"
        :items="people"
...
export default {
  ...
  methods: {
    ...
    /**
     * @vuese
     * Method that outputs a list of all the employees in the database by means of a
     * request to the API.
     */
    list() {
      let me = this;
      ...
      axios
        .get("person/employeeList", config)
    ...
    validator() {
      ...
      if (this.email.length > 50) {
        this.validateMessage.push("Email must have les than 50 characters.");
      }
    ...
    /**
     * @vuese
     * Method that saves employees in the database or edits them through an API request.
     */
    save() {
      let me = this;
      let header = { Token: this.$store.state.token };
    ...
    /**
     * @vuese
     * Method that activates an employee through an API query.
     */
    activate() {
      let me = this;
      let header = { Token: this.$store.state.token };
    ...
    /**
     * @vuese
     * Method that terminates an employee through an API query.
     */
    deactivate() {
      let me = this;
      let header = { Token: this.$store.state.token };
